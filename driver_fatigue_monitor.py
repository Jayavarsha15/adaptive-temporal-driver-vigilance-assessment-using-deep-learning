...
PI_URL = "http://team49.local:5000"

def get_sensor_data() -> dict | None:
    """
    GET /sensor from Raspberry Pi.
    Returns sensor dict or None if unreachable.
    Expected response:
        {
          "gyro": {"gx": value, "gy": value, "gz": value},
          "gps":  {"road_type": "STRAIGHT" or "HILL"}
        }
    """
    try:
        r = requests.get(f"{PI_URL}/sensor", timeout=1.5)
        r.raise_for_status()
        return r.json()
    except Exception:
        return None
...
def send_voice_alert():
    """POST /voice — triggers the ISD1820 module to play warning once."""
    try:
        requests.post(f"{PI_URL}/voice", timeout=1.5)
    except Exception:
        pass


def send_buzzer_alert():
    """POST /buzzer — turns on the buzzer continuously."""
    try:
        requests.post(f"{PI_URL}/buzzer", timeout=1.5)
    except Exception:
        pass


def stop_alert():
    """POST /stop — stops buzzer and resets voice trigger."""
    try:
        requests.post(f"{PI_URL}/stop", timeout=1.5)
    except Exception:
        pass

# ---- Cooldown / rate-limit state for Pi alerts ----
_last_voice_time   = 0.0
_last_buzzer_time  = 0.0
VOICE_COOLDOWN     = 8.0    # seconds between consecutive voice alerts
BUZZER_COOLDOWN    = 3.0    # seconds between buzzer re-triggers

# ==== END RASPBERRY PI COMMUNICATION ====
...
HEAD_DOWN_FRAMES_THR = 12   # ~0.4 s at 30 fps
head_down_frames     = 0

# ==== RASPBERRY PI COMMUNICATION ====
# Mild alert console cooldown (kept from original)
last_alert_time = 0
ALERT_COOLDOWN  = 5   # seconds (prevents spamming console prints)
# ==== END RASPBERRY PI COMMUNICATION ====
...
while True:
    ...
    if res.multi_face_landmarks:
        ...
        if np.all(np.isfinite(image_pts)):
            try:
                success, rvec, tvec = cv2.solvePnP(
                    model_3d, image_pts, cam_mtx, dist_coef,
        ...
        # ============================================================
        # Mild Drowsiness Alert (NORMAL → LOW) — console feedback
        # ============================================================
        ...
        # ==== RASPBERRY PI COMMUNICATION ====
        # Replace original start_alarm() / stop_alarm() calls with
        # context-aware Raspberry Pi alert logic.
        # ----------------------------------------------------------------
        # Decision table:
        #   NORMAL            → stop_alert()
        #   LOW               → send_voice_alert() once (with cooldown)
        #   MEDIUM            → send_voice_alert() occasionally
        #   HIGH / DROWSY
        #   / HEAD DOWN       → query sensors:
        #                         • gyro unstable AND road STRAIGHT
        #                             → send_buzzer_alert()
        #                         • road is HILL (or Pi unreachable)
        #                             → stop_alert()  (false positive risk)
        # ----------------------------------------------------------------

        if alert_level == "NORMAL" and state not in ("DROWSY", "HEAD DOWN"):
            # Driver is fine — make sure everything is silenced
            stop_alert()

        elif alert_level == "LOW":
            # Mild: single voice nudge, do not spam
            if current_time - _last_voice_time > VOICE_COOLDOWN:
                threading.Thread(target=send_voice_alert, daemon=True).start()
                _last_voice_time = current_time

        elif alert_level == "MEDIUM":
            # Moderate: periodic voice reminder (longer gap than LOW)
            medium_cooldown = VOICE_COOLDOWN * 2   # 16 s between prompts
            if current_time - _last_voice_time > medium_cooldown:
                threading.Thread(target=send_voice_alert, daemon=True).start()
                _last_voice_time = current_time

        elif alert_level in ("HIGH",) or state in ("DROWSY", "HEAD DOWN"):
            # Severe: consult sensor data before sounding buzzer
            def _handle_severe_alert():
                global _last_buzzer_time

                sensor = get_sensor_data()   # may return None if Pi is down

                if sensor is None:
                    # Pi unreachable — fail safe: no action to avoid false alarm
                    print("[Pi] Sensor unreachable – skipping buzzer trigger")
                    return
                ...
                gx        = gyro.get("gx", 0)
                gy        = gyro.get("gy", 0)
                road_type = gps.get("road_type", "STRAIGHT")

                gyro_unstable = abs(gx) > 12000 or abs(gy) > 12000

                if road_type == "HILL":
                    # Hill/rough terrain: head drop may be natural — do NOT alarm
                    stop_alert()
                    return

                # road_type == "STRAIGHT" (or unknown)
                if gyro_unstable:
                    # Vehicle is moving erratically on a straight road → BUZZ!
                    now = time.time()
                    if now - _last_buzzer_time > BUZZER_COOLDOWN:
                        send_buzzer_alert()
                        _last_buzzer_time = now
                else:
                    # Stable vehicle on straight road — stop any ongoing alarm
                    stop_alert()

            threading.Thread(target=_handle_severe_alert, daemon=True).start()
        ...
        if yawn_detected:
            ...
            cv2.putText(frame, "YAWN DETECTED", (x_min + 5, y_min - 45),
        ...
        if state == "HEAD DOWN":
            cv2.putText(frame, "COLLAPSE RISK", (30, 255),
